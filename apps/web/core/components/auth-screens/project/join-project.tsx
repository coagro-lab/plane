  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-y-5 bg-custom-background-100 text-center">
      ...
      <h1 className="text-xl font-medium text-custom-text-100">
        {!isPrivateProject ? `Você não é um membro deste projeto ainda.` : `Você não é um membro deste projeto.`}
      </h1>

      <div className="w-full max-w-md text-base text-custom-text-200">
        <p className="mx-auto w-full text-sm md:w-3/4">
          {!isPrivateProject
            ? `Clique no botão abaixo para se juntar a ele.`
      ...
      {!isPrivateProject && (
        <div>
          <Button
            ...
            onClick={handleJoin}
          >
            {isJoiningProject ? "Entrando" : "Clique para se juntar"}
          </Button>
        </div>
      )}
    </div>
